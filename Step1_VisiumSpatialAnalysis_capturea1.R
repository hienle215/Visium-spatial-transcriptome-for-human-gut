...
plot1 <- VlnPlot(data_cap1, features = "nCount_Spatial", pt.size = 0.1) + NoLegend()
plot1
plot2 <- SpatialFeaturePlot(data_cap1, features = "nCount_Spatial") + theme(legend.position = "right")
plot2
...
View(data_cap1_sct)
SpatialFeaturePlot(data_cap1_sct, features = c("IGHG2", "RBP2"))
SpatialFeaturePlot(data_cap1_sct, features = c("HLA-DPA1"))
SpatialFeaturePlot(data_cap1_sct, features = c("GBP1"))
SpatialFeaturePlot(data_cap1_sct, features = "SCG2")

### Gene expression visualization
library(ggplot2)
plot <- SpatialFeaturePlot(data_cap1_sct, features = c("IGHG2", "RBP2")) + theme(legend.text = element_text(size = 0),
...
p1 <- SpatialFeaturePlot(data_cap1_sct, features = c("IGHG2", "RBP2"), pt.size.factor = 1)
p1
p2 <- SpatialFeaturePlot(data_cap1_sct, features = c("IGHG2", "RBP2"), alpha = c(0.1, 1))
p2
p1 + p2
...
p1_1 <- DimPlot(data_cap1_sct_pca_cluster, reduction = "umap", label = TRUE)
p2_1 <- SpatialDimPlot(data_cap1_sct_pca_cluster, label = TRUE, label.size = 3)
p1_1 + p2_1
...
SpatialDimPlot(data_cap1_sct_pca_cluster, interactive = TRUE)
SpatialFeaturePlot(data_cap1_sct_pca_cluster, features = "RBP2", interactive = TRUE) # selecting one gene name in features
LinkedDimPlot(data_cap1_sct_pca_cluster)

### Identification of spatially variable features
de_markers <- FindMarkers(data_cap1_sct_pca_cluster, ident.1 = 3, ident.2 = 4)
SpatialFeaturePlot(object = data_cap1_sct_pca_cluster, features = rownames(de_markers)[0:3], alpha = c(0.1, 1), ncol = 3)

# More details about the identification of vavriable features
install.packages("Rfast2")
data_cap1_sct_pca_cluster = FindSpatiallyVariableFeatures(data_cap1_sct_pca_cluster, assay = "SCT", features = VariableFeatures(data_cap1_sct_pca_cluster)[1:1000], selection.method = "moransi")

# Now we visualize the expression of the top 6 features identified by this measure
top.features = head(SpatiallyVariableFeatures(data_cap1_sct_pca_cluster, selection.method = "moransi"), 3) # error in data frame, no idea where why and how
top.features = head(SpatiallyVariableFeatures(data_cap1_sct_pca_cluster@meta.data, selection.method = "moransi"), 3) # error
top.features = head(SpatiallyVariableFeatures(data_cap1_sct_pca_cluster, selection.method = "markvariogram", decreasing = TRUE), 3)# error
data = data_cap1_sct_pca_cluster
...
) # work well! modified 12.08.2023
SpatialFeaturePlot(data_cap1_sct_pca_cluster, features = top.features, ncol = 3, alpha = c(0.1, 1))
VlnPlot(data_cap1_sct_pca_cluster, features = c("IFI27", "HLA-C", "APOA1", "APOC3", "DEFA5", "ALPI"), ncol = 3)


### SUBSET OUT ANATOMICAL REGIONS. Not yet modification 12.08.2023
...
cortex <- subset(data_cap1_sct_pca_cluster, idents = c(1, 2))
...
plot1 = DimPlot(data_cap1_sct_pca_cluster, reduction = "umap", label = TRUE)
plot2 = SpatialDimPlot(data_cap1_sct_pca_cluster, stroke = 0)
...
DimHeatmap(data_cap1_sct_pca_cluster, dims = 1, cells = 500, balanced = TRUE)

#### NOT YET CHECKED OUT FROM HERE 12.08.2023
