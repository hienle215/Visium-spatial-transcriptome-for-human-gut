...
SpatialFeaturePlot(data_cap2_sct_pca_cluster, features = "GBP1", alpha = c(0.1, 1), max.cutoff = "q95")

# Clustering individually
SpatialDimPlot(data_cap2_sct_pca_cluster, cells.highlight = CellsByIdentities(object = data_cap2_sct_pca_cluster, idents = c(0, 1, 2, 3, 4, 5, 6, 7)), facet.highlight = TRUE, ncol = 4)

SpatialDimPlot(data_cap2_sct_pca_cluster, cells.highlight = CellsByIdentities(object = data_cap2_sct_pca_cluster, idents = c(0, 1, 2, 3,4,5,6,7)), facet.highlight = TRUE, ncol = 4)

FeaturePlot( object = data_cap2_sct_pca_cluster, features = c("IFI27", "HLA-C", "APOA1", "APOC3", "DEFA5", "ALPI"), ncol = 3)

### CORTEX 1 and 7 for the left side samples
# Subset data from visium spatial data based on cluster 1 and 7 for the right sample in capture_area_1
...
anchors <- FindTransferAnchors(reference = reference_SCT, query = cortex, normalization.method = "SCT") # anchors 234
...
### CORTEX_of_cluster_0_&_3
cortex <- subset(data_cap2_sct_pca_cluster, idents = c(0,3))
SpatialFeaturePlot(cortex, features = "HMGCS2", alpha = c(0.1, 1), max.cutoff = "q95", pt.size.factor = 1)

### CORTEX_00_03
# Subset data from visium spatial data based on cluster 0 and 3 for the right sample in capture_area_1
cortex <- subset(data_cap2_sct_pca_cluster, idents = c(0, 3))
...
# match the reference_SCT and cortex for annotating cell types based on clustering
anchors <- FindTransferAnchors(reference = reference_SCT, query = cortex, normalization.method = "SCT") # 336 anchors
predictions.assay <- TransferData(anchorset = anchors, refdata = reference_SCT$category, prediction.assay = TRUE,
                                  weight.reduction = cortex[["pca"]], dims = 1:25, k.weight = 100) 
